...
class OptimizedNHLTrainer:
    ...
    def prepare_features(self, df):
        ...
        df["scoreState"] = df.apply(
            lambda x: (
                "tied"
                if x["home_score"] == x["away_score"]
                else "leading" if x["home_score"] > x["away_score"] else "trailing"
            ),
            axis=1,
        )

        # Power play detection (simplified)
        df["isPowerPlay"] = df["situation_code"].apply(lambda x: 1 if x and (x[:2] in ["51", "41", "31"]) else 0)

        # Shot danger based on location
        df["shotDanger"] = df.apply(
            lambda x: (
                "high"
                if x["shotDistance"] < 20 and x["shotAngle"] < 30
                else "medium" if x["shotDistance"] < 40 else "low"
            ),
            axis=1,
        )
    ...
    def train_xgboost_model(self, df):
        ...
        # Prepare features
        feature_cols = ["shotDistance", "shotAngle", "period", "timeSinceLast", "isRebound", "isRush", "isPowerPlay"]
        ...
        all_features = feature_cols + [col for col in df_encoded.columns if any(cat in col for cat in categorical_cols)]
        ...
        importance_df = (
            pd.DataFrame({"feature": features_to_use, "importance": model.feature_importances_})
